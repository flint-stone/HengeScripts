...
a=0
	...
	cd /var/nimbus/storm/
	sudo bin/storm jar ~/storm-starter-0*.jar storm.starter.StarTopology_2_lexu star remote
	sudo bin/storm jar ~/storm-starter-0*.jar storm.starter.LinearTopology_2_lexu linear remote
	sudo bin/storm jar ~/storm-starter-0*.jar storm.starter.DiamondTopology_2_lexu diamond remote
	sleep 240m
	sudo bin/storm kill star
	sudo bin/storm kill linear
	sudo bin/storm kill diamond
	foldername=$(date +"%Y-%m-%d-%T")+"micro_mix_2"
