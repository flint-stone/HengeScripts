...
a=0
	...
	sudo bin/storm jar ~/storm-starter-0*.jar storm.starter.LinearTopology_4_diurnal_lexu linear remote
	sudo bin/storm jar ~/storm-starter-0*.jar storm.starter.DiamondTopology_4_lexu diamond remote
	sleep 240m
	sudo bin/storm kill star
	sudo bin/storm kill linear
	sudo bin/storm kill diamond
	foldername=$(date +"%Y-%m-%d-%T")+"micro_mix_3_linear_diurnal"
